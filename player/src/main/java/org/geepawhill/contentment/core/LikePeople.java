...
public class LikePeople extends ScriptBuilder<LikePeople> {
	...
	public Script make() {
		script = new Script(
//				new MediaRhythm(new File("d:\\GeePawHillDotOrg\\videos\\likePeople\\joinMtvLikePeople.mp4")));
				new SimpleRhythm());
...
//		}
		letters("GeePaw's *Coaching* Notebook:").format(secondaryJumbo).at(new TopLeft(XMARGIN, YMARGIN)).called("header").appear();
		Font font = Font.font("Chewed Pen BB", FontPosture.ITALIC, 120d);
		Format outlinedPrimary = new Format(primaryJumbo,
				TypeFace.color(Color.WHEAT, color(59, 147, 35),1d),
				TypeFace.font(font, 2d, 1d));
		assume(outlinedPrimary);
		letters("Coaching Software Development?").centered(800, 550).appear();
		letters("Like People!").centered(800, 680).appear();
