...
public class Placeholder implements Actor
{
	...
	public Placeholder(String source, PointPair bounds)
	{
		this.format = new Format(
				TypeFace.color(source, Color.YELLOW, 1d),
				TypeFace.smallFixed(),
				Frames.frame(source, Color.YELLOW,1d, 1d),
				Dash.dash(source, 4d,4d)
				);
	...
	{
		try {
			return new Image(getClass().getResource("/placeholder.jpg").toURI().toString());
		}
		catch(Exception e)
		{
