...
public class PointAssert extends AbstractAssert<PointAssert, Point>
{
	  ...
	  public PointAssert isGoodEnough(Point expected, Offset<Double> offset)
	  {
		  ...
		  if(comparator.compare(actual.x, expected.x)!=0) failWithMessage("X Not close enough. Expected: "+expected.x+" within "+offset+" but was: "+actual.x);
		  if(comparator.compare(actual.y, expected.y)!=0) failWithMessage("y Not close enough. Expected: "+expected.y+" within "+offset+" but was: "+actual.y);
		  return this;
	  }
}
